...
function getStringLength(value) {
  let valueStringLength;
  if (value === null || value === undefined) {
    valueStringLength = 0;
  } else {
    valueStringLength = String(value).length;
  }
  return valueStringLength;
}
...
function removeFirstOccurrences(str, value) {
  const resultRemoved = str.indexOf(value);
  if (resultRemoved === -1) {
    return str;
  }
  return str.slice(0, resultRemoved) + str.slice(resultRemoved + value.length);
}
...
function removeLastOccurrences(str, value) {
  const resultRemoved = str.lastIndexOf(value);
  if (resultRemoved === -1) {
    return str;
  }
  return str.slice(0, resultRemoved) + str.slice(resultRemoved + value.length);
...
function formatTime(minutes, seconds) {
  const minutesCapture = String(minutes).padStart(2, '0');
  const secondsCapture = String(seconds).padStart(2, '0');
  const result = minutesCapture.concat(':', secondsCapture);
  return result;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const strSymbol = strLowerCase.at(i);
    if (vowels.includes(strSymbol)) {
      vowelNumber += 1;
    }
  }
  return vowelNumber;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strArray.length; i += 1) {
    const arrayItem = strArray.at(i);
    const reversedArrayItem = arrayItem.split('').reverse().join('');
    reversedArray = reversedArray.concat(' ', reversedArrayItem);
  }
  return reversedArray.trim();
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    invertCaseText += symbol;
  }
  return invertCaseText;
}
...
function extractNameFromTemplate(value) {
  // let name = value.match(/Hello, (.*) /g);
  // name = name[0].replace('Hello,', '').trim();
  // let surnamePhrase = value.replace(name, '');
  // let surname = surnamePhrase.match(/ (.*)!/g);
  // surname = surname[0].replace('!', '').trim();
  //
  // let name = value.replace('Hello,', '').trim();
...
function extractEmails(str) {
  ...
  let emailsList = '';
  for (let i = 0; i < emailsArray.length; i += 1) {
    if (
      emailsArray[i].match(
        /([a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/
      )[1]
    ) {
      const emailsListItem = emailsArray[i].match(
        /([a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/
      )[0];
      emailsList = emailsList.concat(' ', emailsListItem).trim();
    }
  }
...
function encodeToRot13(str) {
  ...
  let index;
  for (let i = 0; i < str.length; i += 1) {
    if (inputLetters.includes(str.charAt(i))) {
      index = inputLetters.indexOf(str.charAt(i));
      outputText += outputLetters.charAt(index);
    } else {
      outputText += str.charAt(i);
    }
  }
  return outputText;
}
...
function getCardId(value) {
  ...
  if (cardArray.includes(value)) {
    result = cardArray.indexOf(value);
  }
  return result;
}
